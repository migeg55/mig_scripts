...
====================
====================
使用方式：在代理软件配置好国内配置后，复制https://.m.jd.com/myJd/newhome.action 到浏览器打开，在个人中心自动获取cookie，
如果弹出成功则正常使用。否则继续再此页面继续刷新一下记忆。

注：建议通过脚本去获取cookie，若要在BoxJs处手动修改，请按照JSON格式修改（注：可使用此JSON加密https://www.bejson.com/json/format）
示例：[{"userName":"jd_xxx","cookie":"pt_key=AAJ;pt_pin=jd_xxx;"},{"userName":"jd_66","cookie":"pt_key=AAJ;pt_pin=jd_66; "}]
====================
new Env('获取多账号京东Cookie');//此处智能，为自动生成iOS端软件配置文件需要
====================
[中间人]
主机名 = me-api.jd.com
...
==================龙====================
[脚本]
http-request https:\/\/me-api\.jd\.com\/user_new\/info\/GetJDUserInfoUnion script-path=JD_extra_cookie.js, tag=获取多账号登录Cookie

==================浪涌====================
[脚本]
获取多账号京东Cookie = type=http-request,pattern=^https:\/\/me-api\.jd\.com\/user_new\/info\/GetJDUserInfoUnion,requires-body=1,max-size= 0,脚本路径=JD_extra_cookie.js,脚本更新间隔=0
 */

const APIKey = "CookiesJD";
$ = 新 API(APIKey, true);
const CacheKey = `#${APIKey}`;
如果 ($request) GetCookie();
  ...
  尝试 {
    if ($request.headers && $request.url.indexOf("GetJDUserInfoUnion") > -1) {
      ...
      if (CV.match(/(pt_key=.+?pt_pin=|pt_pin=.+?pt_key=)/)) {
        ...
        var DecodeName = decodeURIComponent(用户名);
        ...
        $.notify(
          "用户名：" + DecodeName,
          "",
          tipPrefix + CookieName + "Cookie 成功 🎉"
        );
      } 别的 {
      ...
      $.notify("检查写入京东Cookie失败", "", "请匹配URL或配置内脚本类型‼️");
    }
  } 赶上（或）{
    $.write("", CacheKey);
